...
from hofft.phase_coeffs import coco_to_phis_alphas, b0_to_phis_alphas, rescale_phis_alphas, apply_phase_midpoints
...
hparams = hofft_params(kern_size=(5,5),
                       os=1.5,
                       L=8,
                        # matvec_type=matvec_type3,
                        # matvec_kwargs={'oversamp': 1.25, 
                        #                'width': 2.0, 
                        #                'use_toep': False},
                      #  matvec_type=matvec_cur,
                      #  matvec_kwargs={'cur_rank': 100},
                      #  matvec_type=matvec_rnd,
                      #  matvec_kwargs={'rnd_frac_phis': 0.1, 'rnd_frac_alphas': 0.1},
                       matvec_type=matvec_naive,
                       matvec_kwargs={'spatial_batch_size': 2**10},
                      #  spatial_init='100_alphas',
                       spatial_init='seg',
                       )
print(hparams.os)
...
kern_weights, spatial_factors = als_hofft(trj, phis_nrm, alphas_nrm, hparams, 
                                          im_size_low=(120,120), 
                                          num_als_iter=100)
...
# Recon
img_recon = CG_SENSE_recon(A, ksp, max_iter=10, max_eigen=1.0)
...
plt.subplot(131)
plt.imshow(img_gt.abs().cpu().rot90(), cmap='gray')
plt.axis('off')
plt.subplot(132)
plt.imshow(img_recon.abs().cpu().rot90(), cmap='gray')
plt.axis('off')
plt.subplot(133)
plt.imshow((img_recon.abs() - img_gt.abs()).abs().cpu().rot90(), cmap='gray', vmin=vmax/5)
plt.axis('off')
plt.tight_layout()
plt.show()