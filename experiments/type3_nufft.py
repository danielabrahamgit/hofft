...
class nufft(nn.Module):
    ...
    def adjoint(self, 
                y: torch.Tensor,
                ks: torch.Tensor,
                grid_only: bool = False) -> torch.Tensor:
        """
        ...
        ks : torch.Tensor
            The k-space trajectory, shape (*ks_size, d)
        grid_only : bool
            Whether to only return the gridded data
...
# Build phis alphas
rs = gen_grd(sig_size).to(torch_dev).moveaxis(-1,0) * 2
phis = torch.cat([rs, 2 * rs ** 2, rs ** 3], dim=0)
B = len(phis)
alphas = torch.randn(B, *ks_size, device=torch_dev) * 0.1
alphas[-1] += torch.linspace(0, 3, ks_size[0], device=torch_dev)
