...
class hofft_linop(linop):
    ...
    def __init__(self, 
        ...
        assert spatial_factor.device == torch_dev
        assert spatial_factor.shape[0] == L
    ...
    def forward(self,
        ...
        for c1, c2 in batch_iterator(C, cbs):
            ...
            MSx = einsum(Sx, self.spatial_factor,
                         'C ..., L ... -> C L ...')
            ...
            # Apply kernels
            KFSx = einsum(blocks, self.kern_weights, 'C L K ..., L K ... -> C ...')
            ksp[c1:c2] = KFSx
    ...
    def adjoint(self,
        ...
        for c1, c2 in batch_iterator(C, cbs):
            ...
            Ky = einsum(y, self.kern_weights.conj(), 'C ..., L K ... -> C L ... K')
            ...
            MSFKy = (SFKy * self.spatial_factor.conj()).sum(dim=0)
    ...
    def normal(self,
        ...
        # TODO 1. Rewrite normal operator 
        # TODO 2. topelitz
