...
# Load data
data = torch.load('./paper_experiments/recons/coco_spiral.pt',
                  map_location=torch.device('cpu'),
                  weights_only=True)
imgs_hofft = data['imgs_hofft']
imgs_split = data['imgs_split']
times_hofft = data['times_hofft'] * 1e-3 # ms to s
times_split = data['times_split'] * 1e-3 # ms to s
img_gt = data['img_gt']
Ls = data['Ls']
Ws = data['Ws']
os = data['os']
...
# Compute NRMSEs
nrmses_hofft = torch.zeros(len(Ls), len(Ws), len(Qs), len(Ss))
nrmses_split = torch.zeros(len(Ls), len(Ws), len(Qs), len(Ss))
for l in range(len(Ls)):
    for w in range(len(Ws)):
        for q in range(len(Qs)):
            for s in range(len(Ss)):
                img_hofft = normalize(imgs_hofft[l, w, q, s], img_gt)
                img_split = normalize(imgs_split[l, w, q, s], img_gt)
                nrmses_hofft[l, w, q, s] = (img_hofft.abs() - img_gt.abs()).norm() / img_gt.norm()
                nrmses_split[l, w, q, s] = (img_split.abs() - img_gt.abs()).norm() / img_gt.norm()
                
if len(Qs) == 1 and len(Ss) == 1:
    slc = (slice(None), slice(None), 0, 0)
    imgs_hofft = imgs_hofft[slc]
    imgs_split = imgs_split[slc]
    nrmses_hofft = nrmses_hofft[slc]
    nrmses_split = nrmses_split[slc]
    times_hofft = times_hofft[slc]
    times_split = times_split[slc]
...
plot_errors(nrmses_hofft, nrmses_split, 
            label1='HOFFT', 
            label2='Splitting')
plot_times(times_hofft[..., 0], times_split[..., 0], 
           label1='HOFFT-Decomp', 
           label2='Splitting-Decomp')
plot_times(times_hofft[..., 1], times_split[..., 1], 
           label1='HOFFT-Forward', 
           label2='Splitting-Forward')
